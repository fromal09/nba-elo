...
export default function Goat({ players, onSelectPlayer }) {
  ...
  // Pre-compute avg elos for display
  ...
  const s = {
    ...
    presetHint: { fontSize: 10, opacity: 0.6 },
  ...
  const presetHints = ['40/40/20', '70/25/5', '20/60/20', '15/35/50', '100/0/0']
          ...
          <div style={s.presetGrid}>
            {PRESETS.map((p, i) => (
              <button key={p.label} style={s.presetBtn(activePreset === i)} onClick={() => setPreset(i)}>
                <span>{p.label}</span>
                <span style={s.presetHint}>{presetHints[i]}</span>
              </button>
            ))}
          </div>
        </div>
...
const PRESETS = [
  { label: 'Balanced',          weights: { peak_elo:35, avg_elo:30, longevity:15, playoff_elo:20 } },
  { label: 'Peak Performer',    weights: { peak_elo:55, avg_elo:25, longevity:5,  playoff_elo:15 } },
  { label: 'Ironman',           weights: { peak_elo:20, avg_elo:30, longevity:40, playoff_elo:10 } },
  { label: 'Playoff Legend',    weights: { peak_elo:25, avg_elo:20, longevity:10, playoff_elo:45 } },
  { label: 'Career Dominance',  weights: { peak_elo:20, avg_elo:50, longevity:20, playoff_elo:10 } },
]
...
function computeRawScores(players) {
  const peakElos = players.map(p => p.peak_elo)

  const avgElos = players.map(p => {
    const hist = p.elo_history || []
    return hist.length ? hist.reduce((s, [, v]) => s + v, 0) / hist.length : p.current_elo
  })

  const gps = players.map(p => p.games_played)

  // Playoff avg Elo — games in April–June
  const playoffElos = players.map(p => {
    const hist = p.elo_history || []
    const playoff = hist.filter(([d]) => {
      const m = parseInt(d.slice(5, 7))
      return m >= 4 && m <= 6
    })
    if (!playoff.length) return null
    return playoff.reduce((s, [, v]) => s + v, 0) / playoff.length
  })

  // Players with no playoff data get the median playoff Elo
  const validPlayoff = playoffElos.filter(v => v !== null)
  const medianPlayoff = validPlayoff.sort((a, b) => a - b)[Math.floor(validPlayoff.length / 2)] || 1500
  const filledPlayoff = playoffElos.map(v => v === null ? medianPlayoff : v)

  return { peakElos, avgElos, gps, playoffElos: filledPlayoff }
}
...
export default function Goat({ players, onSelectPlayer }) {
  ...
  const raw = useMemo(() => computeRawScores(eligible), [eligible])

  const normalized = useMemo(() => ({
    peak_elo:    normalize(raw.peakElos),
    avg_elo:     normalize(raw.avgElos),
    longevity:   normalize(raw.gps),
    playoff_elo: normalize(raw.playoffElos),
  }), [raw])
  ...
  const ranked = useMemo(() => {
    ...
    return eligible
      .map((p, i) => {
        const score = DIMENSIONS.reduce((s, dim) => {
          return s + (weights[dim.key] / total) * (normalized[dim.key][i] || 0)
        }, 0)
        return { ...p, goat_score: score * 100 }
      })
      .sort((a, b) => b.goat_score - a.goat_score)
  ...
  const s = {
    wrap:       { display: 'flex', flex: 1, overflow: 'hidden', background: '#f5f3ee', fontFamily: "'DM Sans', sans-serif" },
    sidebar:    { width: 280, flexShrink: 0, background: '#fff', borderRight: '0.5px solid #e0ddd6', display: 'flex', flexDirection: 'column' },
    main:       { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
    sideHead:   { padding: '24px 20px 16px', borderBottom: '0.5px solid #e0ddd6' },
    sideTitle:  { fontFamily: "'DM Serif Display', serif", fontSize: 18, color: '#1a1a1a', marginBottom: 4 },
    sideDesc:   { fontSize: 12, color: '#888', lineHeight: 1.5 },
    presets:    { padding: '16px 20px', borderBottom: '0.5px solid #f0ede8' },
    presetsLbl: { fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: '#bbb', marginBottom: 10 },
    presetGrid: { display: 'flex', flexDirection: 'column', gap: 5 },
    presetBtn:  (active) => ({
    ...
    sliderTop:  { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 },
    sliderLabel:{ fontSize: 13, fontWeight: 500, color: '#333' },
    sliderDesc: { fontSize: 11, color: '#aaa', marginBottom: 6 },
    sliderVal:  { fontSize: 12, fontWeight: 600, color: '#1a2e1a' },
    totalRow:   { padding: '12px 20px', borderTop: '0.5px solid #e0ddd6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    totalLabel: { fontSize: 11, color: '#aaa' },
    totalVal:   (ok) => ({ fontSize: 13, fontWeight: 600, color: ok ? '#2d8a5a' : '#c94040' }),
    pageHeader: { padding: '24px 32px 16px' },
  ...
  return (
    <div style={s.wrap}>
      <div style={s.sidebar}>
        <div style={s.sideHead}>
          <div style={s.sideTitle}>GOAT Rankings</div>
          <div style={s.sideDesc}>Adjust weights to define greatness. Scores are normalized 0–100.</div>
        </div>
      ...
      <div style={s.main}>
        <div style={s.pageHeader}>
          <h1 style={s.pageTitle}>{activePreset >= 0 ? PRESETS[activePreset].label : 'Custom GOAT'}</h1>
          <p style={s.pageDesc}>Min 200 GP · {eligible.length.toLocaleString()} players · composite score 0–100</p>
        </div>
          ...
          <table style={s.table}>
            <thead style={s.thead}>
              <tr>
                ...
                <th style={{ ...s.th, ...s.thR }}>Avg Elo</th>
                <th style={{ ...s.th, ...s.thR }}>GP</th>
                <th style={{ ...s.th, ...s.thR }}>Playoff Avg Elo</th>
              </tr>
            </thead>
            <tbody>
              {slice.map((p, i) => {
                ...
                const score    = p.goat_score
                const barColor = rank === 1 ? '#c9920a' : rank <= 5 ? '#2d8a5a' : rank <= 25 ? '#1a5fa8' : '#bbb'
                const avgElo   = p.elo_history?.length
                  ? Math.round(p.elo_history.reduce((s, [, v]) => s + v, 0) / p.elo_history.length)
                  : Math.round(p.current_elo)
                const playoffHist = (p.elo_history || []).filter(([d]) => { const m = parseInt(d.slice(5,7)); return m >= 4 && m <= 6 })
                const playoffAvg  = playoffHist.length
                  ? Math.round(playoffHist.reduce((s, [, v]) => s + v, 0) / playoffHist.length)
                  : null
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ ...s.td, textAlign: 'right', fontSize: 13, color: '#555', fontVariantNumeric: 'tabular-nums' }}>
                      {avgElo.toLocaleString()}
                    </td>
                    <td style={{ ...s.td, textAlign: 'right', fontSize: 13, color: '#555', fontVariantNumeric: 'tabular-nums' }}>
                      {p.games_played.toLocaleString()}
                    </td>
                    <td style={{ ...s.td, textAlign: 'right', fontSize: 13, color: '#555', fontVariantNumeric: 'tabular-nums' }}>
                      {playoffAvg ? playoffAvg.toLocaleString() : '—'}
                    </td>
                  </tr>
                )
              })}
